...
export default function EditDialog(props: {
  ...
  const { showModal, shopId } = props;
  const [imageFile, setImageFile] = useState<File | null>(null); // 新規画像用
  const [imageUrl, setImageUrl] = useState<string | null>(null); // 既存画像URL用
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (data) {
        ...
        setValue('shop_review', data.shop_review ?? '');
        setValue('instagram_id', data.instagram_id ?? '');

        // favorite_menusがオブジェクトの配列であることを確認し、valueを取り出す
        setValue(
          'favorite_menus',
          Array.isArray(data.favorite_menus)
            ? data.favorite_menus.map((menu: any) => ({
                value: menu.value || menu, // オブジェクトか単なる文字列かに対応
              }))
            : [],
        );
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      const image_url = imageFile ? await uploadImage(imageFile) : imageUrl; // 既存の画像URLがあれば使用

      // favorite_menusの配列を文字列の配列に変換してから送信
      const favoriteMenusValues = data.favorite_menus.map((menu) => menu.value);
      ...
      const { data: supabaseData, error } = await supabase
        ...
        .update({
          ...
          favorite_menus: favoriteMenusValues, // ここで文字列の配列にする
              ...
              <div className="mb-4 ml-[15%] w-[76%]">
                ...
                {fields.map((field, index) => (
                  <div key={field.id} className="mb-2 flex items-center gap-1">
                    <input
                      {...register(`favorite_menus.${index}.value`)} // 正しいプロパティにアクセス
                      type="text"
                      placeholder="好きなメニュー"
